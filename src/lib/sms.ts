...
export async function sendSmsOtp(phone: string, otpCode: string): Promise<SmsResult> {
  ...
  if (FAST2SMS_KEY) {
    try {
      ...
      // 1A. Primary: Fast2SMS OTP route ('otp') - Costs only ₹0.25 / SMS (Requires Fast2SMS website verification)
      ...
      if (otpData && (otpData.return === true || otpData.status_code === 200)) {
        console.log(`[SMS] Fast2SMS OTP route (₹0.25) dispatched to +91${cleanPhone}:`, otpData.message || otpData.request_id);
        return { success: true, gateway: 'Fast2SMS (₹0.25 OTP)', messageId: otpData.request_id || otpData.message?.[0] };
      }

      console.warn('[SMS] Fast2SMS OTP route not verified yet (falling back to Quick SMS):', otpData?.message || otpData);

      // 1B. Fallback: Quick SMS route ('q') - Instant delivery without verification, but Fast2SMS charges ₹5.00 / SMS
      ...
      if (qData && (qData.return === true || qData.status_code === 200)) {
        ...
        return { success: true, gateway: 'Fast2SMS (Quick Route)', messageId: qData.request_id || qData.message?.[0] };
      }
