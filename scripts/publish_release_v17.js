...
async function publishRelease() {
  const apkPath = path.join(__dirname, '../../LaundryFresh-v1.0.17-release.apk');
  ...
  const s3Key = 'releases/LaundryFresh-v1.0.17-release.apk';
  ...
  const versionName = 'v1.0.17';
  const versionCode = 17;
  const fileName = 'LaundryFresh-v1.0.17-release.apk';
  const releaseNotes = 'v1.0.17: Pure Google Firebase Phone Authentication (Fast2SMS completely disabled), Categories crash fix (null toLowerCase error resolved), strictly 4 items per row full-rounded circular grids, Referral banner and WhatsApp sharing.';
  ...
  console.log('\n========================================');
  console.log('RELEASE v1.0.17 PUBLISHED SUCCESSFULLY!');
