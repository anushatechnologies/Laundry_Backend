...
async function publishRelease() {
  const apkPath = path.join(__dirname, '../../LaundryFresh-v1.0.45-release.apk');
  ...
  const s3Key = 'releases/LaundryFresh-v1.0.45-release.apk';
  ...
  // Also upload latest alias so existing apps downloading "LaundryFresh.apk" always get the newest
  try {
  ...
  const versionName = 'v1.0.45';
  const versionCode = 45;
  const fileName = 'LaundryFresh-v1.0.45-release.apk';
  const releaseNotes = 'v1.0.45: Instant 0ms search (no loading delay), search header top gap fixed, Live Chat opens instantly with smart RAMYA auto-replies, chat input safe above Android navigation bar, background message delivery.';
  ...
  console.log('\n========================================');
  console.log('RELEASE v1.0.45 PUBLISHED TO AWS SUCCESSFULLY!');
