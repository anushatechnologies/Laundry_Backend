...
async function publishRelease() {
  const apkPath = path.join(__dirname, '../../LaundryFresh-v1.0.36-release.apk');
  ...
  const s3Key = 'releases/LaundryFresh-v1.0.36-release.apk';
  ...
  const versionName = 'v1.0.36';
  const versionCode = 36;
  const fileName = 'LaundryFresh-v1.0.36-release.apk';
  const releaseNotes = 'v1.0.36: Sticky bottom checkout bar with Total on left and Proceed to Pickup & Slots on right; direct shopping bag access from garment detail screens; real-time two-way live chat with high-priority FCM notifications; clear chat history; instant referral bonuses.';
  ...
  console.log('\n========================================');
  console.log('RELEASE v1.0.36 PUBLISHED SUCCESSFULLY!');
  console.log(`Download URL: ${s3Url}`);
  console.log(`Latest Alias: https://${BUCKET}.s3.${REGION}.amazonaws.com/releases/LaundryFresh.apk`);
  console.log('========================================\n');
}
