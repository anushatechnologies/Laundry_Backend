...
async function publishRelease() {
  const apkPath = path.join(__dirname, '../../LaundryFresh-v1.0.52-release.apk');
  ...
  const s3Key = 'releases/LaundryFresh-v1.0.52-release.apk';
  ...
  const versionName = 'v1.0.52';
  const versionCode = 52;
  const fileName = 'LaundryFresh-v1.0.52-release.apk';
  const releaseNotes = 'v1.0.52: Simultaneous multi-filter (combined Price Low-High and Alphabetical A-Z sort), Alphabetical letter scrubber, Kurta & Dhoti high-definition photo loading, and dark mode policy text readability enhancements.';
  ...
  await connection.execute(
    ...
    [releaseId, versionName, versionCode, fileName, s3Url, stat.size, releaseNotes, 0]
  );
  ...
  console.log('\n========================================');
  console.log('RELEASE v1.0.52 PUBLISHED TO AWS & DB SUCCESSFULLY!');
