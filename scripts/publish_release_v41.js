...
async function publishRelease() {
  const apkPath = path.join(__dirname, '../../LaundryFresh-v1.0.41-release.apk');
  ...
  const s3Key = 'releases/LaundryFresh-v1.0.41-release.apk';
  ...
  const versionName = 'v1.0.41';
  const versionCode = 41;
  const fileName = 'LaundryFresh-v1.0.41-release.apk';
  const releaseNotes = 'v1.0.41: Instant 0ms category opening with pre-warmed catalog, floating cart bar restricted to shopping screens only with clean 16px gap above the footer pill, and enhanced safe-area bottom elevation.';
  ...
  console.log('\n========================================');
  console.log('RELEASE v1.0.41 PUBLISHED TO AWS SUCCESSFULLY!');
